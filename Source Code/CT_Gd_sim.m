...
for k=0:1/180*pi:2*pi-1/180*pi
    ...
    l_Gd=zeros(length(x_det),4);
    miu_GdL=zeros(length(x_det),4);
    miu_GdR=zeros(length(x_det),4);
    for n_at=1:4
        l_Gd(:,n_at)=phantom_d(-disDec,0,x_det,y_det,X_Gd(n_at),Y_Gd(n_at),r);
        miu_GdL(:,n_at)=(exp(interp1(eli,log(uli),EpL))+exp(interp1(egd,log(ugd),EpL))*rou_Gd(n_at))*(1+rou_Gd(n_at));
        miu_GdR(:,n_at)=(exp(interp1(eli,log(uli),EpR))+exp(interp1(egd,log(ugd),EpR))*rou_Gd(n_at))*(1+rou_Gd(n_at));
    end
    
    %Calculate count after attenuation
    IL=NpL.*exp(-sum(l_Gd.*miu_GdL,2)-(l_PMMA-sum(l_Gd,2)).*miu_PMMAL);
    IR=NpR.*exp(-sum(l_Gd.*miu_GdR,2)-(l_PMMA-sum(l_Gd,2)).*miu_PMMAR);
...
sig1=sum(N(find(E==40):find(E==45)))/sum(NpL);
sig2=sum(N(find(E==55):find(E==60)))/sum(NpR);
n_detL=n_detL*sig1;
n_detR=n_detR*sig2;
...
%% Save
save projdata\uatt_90keV_double u_map1 u_map2
